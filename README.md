...
- ```dbname```: название базы данных в Postgres.
...
Проект выполнен на Python 3.9.10 в PyCharm CE (64-bit, Linux Ubuntu). Код для запуска чат-бота находится в файле main.py, а библиотеки для установки - в файле requirements.txt.
...
- `data`: папка с данными, относительно которых формируется БД в Postgres:
  - `database_csv`: папка, содержащая CSV-файл с 4000 уникальными английскими словами (`database.csv`). Относительно `database.csv` заполняется первоначальная БД в Postgres.
...
- **`words`**: таблица с данными по английским словам.
...
- **`users_words`**: таблица, связывающая пользователей со словами.
  ...
  - `is_user_word`: факт добавления слова пользователем.
    ...
    - `False` - слово добавлено разработчиком приложения.
  - `date_added`: дата добавления пользователем слова в личной БД.
  - `date_deleted`: дата удаления пользователем слова в личной БД.
...
```python
def main_function(dbname: str, user: str, password: str, token: str) -> None:
    pass
...
if __name__ == '__main__':
    ...
    USER = 'postgres'
    PASSWORD = 'postgres'
    TOKEN = '1010101010:AAAaaaAAaaaa1aAaAaAAAaAA-aaAAAaAaO1'
